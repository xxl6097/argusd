...
DEAUTH + WIFI_DISCONNECT + MACTABLE_DELETE 几乎同时产生。这三条 hint 派生 3 个 worker，但 `handleDisconnectHint` 入口的 `disconnectInFlight` 集合保证只有第一个进入 500ms Sleep + ping 流程，第二/第三个直接发 `DISCONNECT_SKIP_INFLIGHT` 决策返回。第一个 worker 在 Sleep 结束后调用 `drainHintsFor` 清空 channel 中该 MAC 的尚未派发的 hint。
