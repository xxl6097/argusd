...
logread -f (runSyslog goroutine)          每 PollInterval=1s
  ...
  Del Sta:<mac>         (MACTABLE_DELETE)    └── filterAlive     → alive
    │                                           │
  syslogHint{Disconnect:true}               loadARPStates(ctx)
    │                                           │
    ▼ (syslogHints channel cap=256)          diff(known, cur, apRaw)
    │                                           │
  runSyslogConsumer                      ┌──────┼──────┬────────┬──────┐
  (最多 16 并发 worker)                   ▼      ▼      ▼        ▼       ▼
    │                                 ping    AP感知  RSSI     ARP    冷却期
  handleDisconnectHint                 过滤    息屏    分级     加速    + 抖动
    │                                    │
  ① 不在 known → DISCONNECT_IGNORE
  ② Sleep 500ms (漫游窗口)
  ③ drainHintsFor (清同 MAC 重复 hint)
  ④ ping 可达? → DISCONNECT_PING_OK (漫游)
  ⑤ 再次检查 known (可能被 diff 删除)
  ⑥ 抖动窗口? → FLAP_SUPPRESS_OFFLINE
  ⑦ OFFLINE_EMIT → EventOffline + cooldown
```

共享状态 `known` / `misses` / `offlineCooldown` / `lastEventAt` 通过 `stateMu` 串行化。
...
```go
func (w *Watcher) handleDisconnectHint(ctx, mac, onEvent) {
    ...
    if rssi != 0 && rssi < cfg.WeakRSSI {        // 默认 -80
        ...
        if misses[mac] >= threshold {
            if _, inCD := cooldown[mac]; inCD {
                ...
                emitIfNotSuppressed(EventOffline, d)    // 含 FlapSuppression
            }
            cooldown[mac] = now
            delete(known, mac)
            delete(misses, mac)
...
1. 无条件写入 `cooldown[mac] = now`
2. 从 `known` / `misses` 移除设备
3. 若**已**在冷却期内，走 `COOLDOWN_SUPPRESS_OFFLINE` 分支，**不重复**发射 `EventOffline`（但仍刷新 cooldown）
...
设 `OfflineCooldown = time.Nanosecond` 可实际禁用冷却期（注意 0 值会被 `WithConfig` 忽略）。
...
- 设 `FlapSuppressionWindow = 0` 完全关闭
...
DEAUTH + WIFI_DISCONNECT + MACTABLE_DELETE 几乎同时产生。第一条 hint 启动 worker 进入 500ms 等待，`drainHintsFor` 在 Sleep 结束后清空 channel 中该 MAC 的**后续**hint。

⚠️ 注意：`drainHintsFor` 只能处理**已入队**的 hint。若第二/第三条在 500ms 内尚未入队（极端情况），会被独立 worker 处理；但它们再次进入 `handleDisconnectHint` 时，`known[mac]` 已被第一条清除，走 `DISCONNECT_IGNORE_UNKNOWN` 分支。
